...
class OJT:
    ...
    def play(self, text, speed=1.0, half_tone=0.0, file_name=None):
        x, sr = self.tts(text, speed=speed, half_tone=half_tone)
        if file_name: wavfile.write(file_name, sr, x.astype(numpy.int16)) # test.wav
        return sa.play_buffer(x.astype(numpy.int16), 1, 2, 48000) # https://github.com/r9y9/pyopenjtalk/blob/master/pyopenjtalk/__init__.py#L133
